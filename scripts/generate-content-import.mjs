...
// Simplified output is generated by the front-end at runtime via opencc-js.
// We deliberately do NOT pre-compute it here, to keep R2 storage under the
// Cloudflare free-tier cap. All *_simplified fields are written as `null`
// placeholders to preserve the response schema; the front-end falls back to
// opencc when the field is null.
...
function isPublicPopularWork(work) {
  const title = norm(work.title_traditional);
  const author = norm(work.author_name_traditional);
...
function publicWorkKey(work) {
  const author = work.author_name_traditional;
  const title = work.genre === "ci" ? work.rhythmic || work.title_traditional : work.title_traditional;
...
function addCollection({ slug, title, type, dynasty = null, description = "", sourcePath = "", sortOrder = 0 }) {
  ...
  if (!collections.has(id)) {
    collections.set(id, {
      ...
      titleSimplified: null,
      type,
      dynasty,
      descriptionTraditional: description,
      descriptionSimplified: null,
      sourcePath,
      sortOrder,
      itemCount: 0,
    });
...
function addAuthor(name, dynasty, description = "", source = "", sourceId = "") {
  ...
  if (!authors.has(id)) {
    ...
    authors.get(id).descriptionSimplified = null;
  }
  return id;
}
...
function addWork(input) {
  ...
  works.push({
    ...
    wordCount: plainTraditional.replace(/\s/g, "").length,
  ...
  addWorkContent(workId, {
    ...
    title_simplified: null,
    author_id: authorId,
    author_name_traditional: input.author || "",
    author_name_simplified: null,
    dynasty: input.dynasty || null,
    genre: input.genre,
    ...
    strains: input.strains || null,
    rank_metrics: input.rank?.metrics || null,
  });
...
function importPoetryLike() {
  ...
  const singleSpecs = [
    ...
    ["宋词/宋词三百首.json", "songci300", "宋词三百首", "ci", "宋", "词"],
  ];
  ...
  // 花间集 — 10 juan files + a preface (skip the preface: it carries no poem body).
  if (!works.length || works.length < limit) {
    for (const rel of list("五代诗词/huajianji", (f) => /^huajianji-\d+-juan\.json$/.test(f))) {
      if (works.length >= limit) return;
      // Reuse the nantang collection so 花间集 shows up under "五代词" alongside 南唐二主.
      ...
      for (const item of readJson(rel)) {
        if (works.length >= limit) return;
        if (!item.paragraphs || !Array.isArray(item.paragraphs)) continue;
        addWork({
          ...item,
          author: item.author || "",
          paragraphs: item.paragraphs,
          title: item.title || item.rhythmic || "花间集",
          dynasty: "五代",
          genre: "ci",
          formType: "词",
          rhythmic: item.rhythmic || null,
  ...
  // 御定全唐詩 — 900 juan files. Stream through them to avoid loading all
  // records in RAM. volume + no# give a stable in-corpus order.
  if (!works.length || works.length < limit) {
    const yuandingFiles = list("御定全唐詩/json", (f) => /^\d+\.json$/.test(f));
    if (yuandingFiles.length) {
      const collectionId = addCollection({
        slug: "yudingquantangshi",
        title: "御定全唐詩",
        type: "poetry",
        dynasty: "唐",
        sourcePath: yuandingFiles[0],
        sortOrder: 11,
      });
      for (const rel of yuandingFiles) {
        if (works.length >= limit) return;
        for (const item of readJson(rel)) {
          if (works.length >= limit) return;
          if (!item.paragraphs || !Array.isArray(item.paragraphs)) continue;
          addWork({
            ...
            author: item.author || "",
            paragraphs: item.paragraphs,
            title: item.title || `${item.volume || ""}${item["no#"] ? "·" + item["no#"] : ""}`,
            dynasty: "唐",
            genre: "shi",
            formType: "诗",
            rhythmic: null,
            collectionId,
            collectionTitle: "御定全唐詩",
            sourcePath: rel,
            notes: Array.isArray(item.notes) ? item.notes.filter((n) => typeof n === "string" && n.trim()).join("\n") : null,
            metadata: { volume: item.volume || null, ordinal: item["no#"] || null },
            annotation: item.biography || item.notes?.join?.("\n") || null,
          });
        }
      }
    }
...
function importStrains() {
  ...
  // strains files mirror both poet.tang.*.json and poet.song.*.json names.
  for (const rel of list("strains/json", (f) => /^poet\.(tang|song)\.\d+\.json$/.test(f))) {
    let items;
    try {
      items = readJson(rel);
    } catch (err) {
      console.warn(`Skipping malformed strains file ${rel}: ${err.message}`);
      continue;
    }
...
function applyStrainsToWorks() {
  // Group strain rows by workId into per-work pattern arrays.
  ...
  for (const [workId, rows] of byWork) {
    ...
    for (const shard of workContentByShard.values()) {
      if (shard[workId]) {
        shard[workId].strains = patterns;
        break;
      }
    }
  }
...
function writeCatalogIndexes() {
  ...
  const sortedWorks = works
    ...
    .sort((a, b) => {
      ...
      // tie-break: works with no popularity score fall back to longer body so
      // newly-imported corpora (yuandingquantangshi, shijing, lunyu...) appear
      // before 1-line poems that already got a score.
  ...
  })).sort((a, b) => `${a.dynasty || ""}${a.name_traditional}`.localeCompare(`${b.dynasty || ""}${b.name_traditional}`, 'zh-Hant'));
  ...
  const collectionRows = [...collections.values()]
    ...
    .sort((a, b) => (a.sort_order - b.sort_order) || a.title_traditional.localeCompare(b.title_traditional, 'zh-Hant'));
